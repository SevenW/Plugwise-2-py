...
#TODO: Remove very specific strategy to prevent disk on all the time
#        it logs to /tmp and rsyncs to disk hourly.
...
# history logs rate < 60 minutes are logged to persitent, as updates happen every hour.
# # /tmp/pw-* quick log files may be gone
# perfile = perpath + logpre + now.date().isoformat() + '*' + logpost
# cmd = "rsync -aXuv " +  perfile + " " + tmppath
# subprocess.call(cmd, shell=True)
...
class PWControl(object):
    ...
    def __init__(self):
        ...
        self.device = Stick(port="/dev/usb/ttyUSB0", timeout=1)
    ...
    def rsync_to_persistent(self):
        ...
        # history logs rate < 60 minutes are logged to persitent, as updates happen every hour.
        # # /tmp/pw-* quick log files may be gone
        # tmpfile = tmppath + logpre + '*' + logpost
        # cmd = "rsync -aXv " +  tmpfile + " " + perpath
        # subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    ...
    def cleanup_tmp(self):
        ...
        # history logs rate < 60 minutes are logged to persitent, as updates happen every hour.
        # tmpfiles = tmppath + logpre + '*' + logpost
        # for fn in glob.iglob(tmpfiles):
             # if time.time()-os.path.getmtime(fn) > cleanage:
                # os.unlink(fn)
                ...
                if not dt is None:                
                    ...
                    if c.interval <60:
                        if prev_dt.date() != dt.date():
                            ...
                            # pesistent iso tmp
                            # newfname= tmppath + logpre + ndate + '-' + mac + logpost
    ...
    def run(self):
        ...
        global hour
        global minute
        i=self.byname['zonnepanelen-1710']
        c=self.circles[i]
